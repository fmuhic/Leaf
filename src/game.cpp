...
Game::Game(i32 maxEntityCount, Leaf* leaf, EntitySystem* entitySystem) {
    entities.reserve(maxEntityCount);
    for (i32 i = 0; i < maxEntityCount; ++i)
        entities.push_back(Entity{});

    geometry = new Geometry(maxEntityCount, nullptr);
    physics = new Physics();
    this->leaf = leaf;
...
void Game::changeScene([[maybe_unused]]Example *newExample) {
    // reset();
    // delete example;
    // example = newExample;
